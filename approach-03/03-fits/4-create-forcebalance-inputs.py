...
def main(
    ...
    with portal_client_manager(lambda _: client):
        if protein_record_ids_path is None:
            torsion_profile_target_schemas = [
                torsiondrive_target_schema_type.construct(
    ...
    print(f"Created {len(torsion_profile_target_schemas)} torsion profile schemas.")
    ...
    for smirks in valence_smirks["Angles"]:
        # use AngleSMIRKS.construct to skip validation
        # the current chemper backend used to validate smiles
        # doesn't do well with rings
        if smirks in linear_angle_smirks:
            parameter = AngleSMIRKS.construct(smirks=smirks, attributes={"k"})
        else:
            parameter = AngleSMIRKS.construct(smirks=smirks, attributes={"k", "angle"})
        target_parameters.append(parameter)
    
    for smirks in valence_smirks["Bonds"]:
        target_parameters.append(BondSMIRKS.construct(smirks=smirks, attributes={"k", "length"}))
    
    force_field = ForceField(force_field_path)
    print(f"Loaded {force_field_path}")
    ...
    optimization_schema = OptimizationSchema(
        ...
        stages=[
            OptimizationStageSchema.construct(
                optimizer=optimizer,
                targets=targets,
